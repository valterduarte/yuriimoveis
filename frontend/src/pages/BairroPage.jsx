...
const NEIGHBORHOOD_PROPERTIES_LIMIT = 50
...
export default function BairroPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/api/imoveis?bairro=${encodeURIComponent(neighborhoodName)}&limit=${NEIGHBORHOOD_PROPERTIES_LIMIT}`, { signal: controller.signal })
      .then(res => {
        const fetched = res.data.imoveis || []
        setProperties(fetched)
        setTotal(res.data.total || 0)
        if (fetched.length === 0) setNotFound(true)
      })
        ...
        jsonLd={[
          {
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 3, name: neighborhoodName, item: `${SITE_URL}${pageUrl}` },
            ],
          },
          !notFound && {
            '@context': 'https://schema.org',
            ...
            itemListElement: properties.map((property, i) => ({
              '@type': 'ListItem',
              position: i + 1,
              url: `${SITE_URL}/imoveis/${property.titulo?.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}-${property.id}`,
