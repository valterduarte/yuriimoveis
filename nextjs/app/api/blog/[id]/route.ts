...
import { buildBlogUpdateAssignments } from '../../../../lib/blogUpdate'
...
export const PUT = withErrorHandler('PUT /api/blog/[id]', async (request: NextRequest, context: RouteContext) => {
  ...
  const { fields, values } = buildBlogUpdateAssignments(updates)
  ...
  const result = await getDb().query(
    `UPDATE blog_posts SET ${fields.join(', ')} WHERE id = $${values.length + 1} RETURNING id`,
    [...values, id]
  )
  if (!result.rows[0]) return notFoundJson('Post não encontrado')
