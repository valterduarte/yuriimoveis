...
export default function VideoSection({ videoUrl, isUploading, onUpload, onRemove }: VideoSectionProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 p-6">
      <h2 className="text-[10px] font-bold uppercase tracking-widest text-dark mb-1">Vídeo (opcional)</h2>
      <p className="text-[10px] text-gray-500 mb-4">Um vídeo de tour do imóvel. Se vazio, a seção não aparece na página.</p>
      ...
      {videoUrl && (
        <div className="relative group mt-4">
          <video src={videoUrl} controls preload="none" className="w-full max-h-48 object-cover border border-gray-200 bg-black" />
          <button
            type="button"
            onClick={onRemove}
